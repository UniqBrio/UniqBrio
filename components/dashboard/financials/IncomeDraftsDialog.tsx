...
interface IncomeDraftsDialogProps {
  ...
  onEditDraft: (draft: IncomeFormData, draftId: string) => void;
}
...
export function IncomeDraftsDialog({ 
  open, 
  onOpenChange, 
  onEditDraft
  ...
  const loadDrafts = async () => {
    ...
    setError(null);
    try {
      const draftsData = await IncomeDraftsAPI.getAllDrafts();
      setDrafts(draftsData);
      
      // Trigger event with initial draft count
      IncomeDraftsAPI.triggerDraftsUpdatedEvent(draftsData, 'migrated');
      ...
      try {
        const refreshed = await IncomeDraftsAPI.getAllDrafts();
        setDrafts(refreshed);
        // Notify other components with the full, authoritative list
        IncomeDraftsAPI.triggerDraftsUpdatedEvent(refreshed, 'migrated');
      } catch (refreshError) {
        console.warn('Failed to refresh drafts after migration:', refreshError);
      }
    } catch (error) {
      console.error('Error loading income drafts:', error);
      setError('Failed to load drafts');
      setDrafts([]);
      // Trigger event with empty array
      IncomeDraftsAPI.triggerDraftsUpdatedEvent([], 'migrated');
    } finally {
      setLoading(false);
    }
  };
  ...
  const migrateLegacyDrafts = async () => {
    try {
      const savedDrafts = localStorage.getItem('income-drafts');
      const legacyDraft = localStorage.getItem('draft-income');
      
      if (savedDrafts) {
        const parsedDrafts = JSON.parse(savedDrafts);
        for (const draft of parsedDrafts) {
          await IncomeDraftsAPI.createDraft({
            name: draft.name || IncomeDraftsAPI.generateDraftName(draft.data),
            category: draft.category || draft.data.incomeCategory || 'Uncategorized',
            amount: draft.amount || draft.data.amount || '0',
            data: draft.data
          });
        }
        localStorage.removeItem('income-drafts');
      }
      
      if (legacyDraft && !savedDrafts) {
        const parsed = JSON.parse(legacyDraft);
        const name = IncomeDraftsAPI.generateDraftName(parsed);
          
        await IncomeDraftsAPI.createDraft({
          name,
          category: parsed.incomeCategory || 'Uncategorized',
          amount: parsed.amount || '0',
          data: parsed
        });
        localStorage.removeItem('draft-income');
      }
    } catch (error) {
      console.error('Error migrating legacy income drafts:', error);
    }
  };
  ...
  const handleDeleteClick = (draft: IncomeDraft) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await IncomeDraftsAPI.deleteDraft(draftToDelete.id);
      const updatedDrafts = drafts.filter(d => d.id !== draftToDelete.id);
      setDrafts(updatedDrafts);
      
      // Trigger custom event for other components to update
      IncomeDraftsAPI.triggerDraftsUpdatedEvent(updatedDrafts, 'deleted');
      ...
      toast({
        title: "? Delete Failed",
        description: "Unable to delete income draft. Please try again.",
        variant: "destructive",
        duration: 4000,
      });
    } finally {
  ...
  const handleEditDraft = (draft: IncomeDraft) => {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Manage your saved income drafts. Click to edit draft or delete. ({drafts.length} draft{drafts.length !== 1 ? 's' : ''} found)
                </p>
              </div>
          ...
          <div className="overflow-y-auto flex-1 py-4 min-h-0">
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                <p>No income drafts found</p>
                <p className="text-sm">Start adding an income and save as draft to see them here</p>
              </div>
            ) : (
