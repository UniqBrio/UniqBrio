...
export default function InstructorProfile({
  ...
  const { instructors: storeInstructors, deleteInstructor: deleteFromStore, getFormById, updateInstructor, loading: instructorsLoading } = useInstructors()

  useEffect(() => {
    const mapped: Instructor[] = storeInstructors.map((s: StoreInstructor) => {
      // Use backend-provided denormalized fields directly
      const cohortNamesArray = (s.cohortName || '')
        .split(',')
        .map(n => n.trim())
        .filter(Boolean)
      ...
      return {
        ...
        // Use backend-computed values directly (already filtered for Active cohorts)
        courseAssigned: s.courseAssigned || undefined,
        cohortName: s.cohortName || undefined,
        courseIds: s.courseIds || undefined,
        cohortIds: s.cohortIds || undefined,
        cohortNames: cohortNamesArray,
      }
    })
    setInstructors(mapped)
  }, [storeInstructors])

  // Also listen to global sync events to update instantly when another hook instance adds/edits
  useEffect(() => {
    const handler = (e: Event) => {
      try {
        const detail = (e as CustomEvent<{ items: any[] }>).detail
        const arr = detail?.items || []
        const mapped: Instructor[] = arr.map((it: any) => {
          const inst = it?.instructor || it
          const id = it?.id || inst?.id
          
          // Use backend-provided denormalized fields directly
          const cohortNamesArray = (inst?.cohortName || '')
            .split(',')
            .map((n: string) => n.trim())
            .filter(Boolean)
          ...
          return {
            ...
            phone: inst?.phone,
            gender: inst?.gender,
            yearsOfExperience: inst?.experience,
            contractType: inst?.contractType,
            jobLevel: inst?.jobLevel,
            dateOfBirth: (inst as any)?.dateOfBirth,
            joiningDate: (inst as any)?.joiningDate,
            // Use backend-computed values directly
            courseAssigned: inst?.courseAssigned || undefined,
            cohortName: inst?.cohortName || undefined,
            courseIds: inst?.courseIds || undefined,
            cohortIds: inst?.cohortIds || undefined,
            cohortNames: cohortNamesArray,
          } as Instructor
        }).filter(x => x.instructorId)
        if (mapped.length) setInstructors(mapped)
      } catch {}
    }
  ...
  const backfillFormFromInstructor = (inst: StoreInstructor): InstructorFormData => {
    ...
    return {
      ...
      email: inst.email || "",
      phone: inst.phone || "",
      phoneCountryCode: (inst as any).phoneCountryCode || "+91",
      ...
      paymentInfo: {
        classCount: (inst as any).paymentInfo?.classCount || "",
        frequency: (inst as any).paymentInfo?.frequency || "",
        hourlyRate: (inst as any).paymentInfo?.hourlyRate || "",
        bankName: (inst as any).paymentInfo?.bankName || "",
        accountHolder: (inst as any).paymentInfo?.accountHolder || "",
        accountNumber: (inst as any).paymentInfo?.accountNumber || "",
        ifsc: (inst as any).paymentInfo?.ifsc || "",
        branchAddress: (inst as any).paymentInfo?.branchAddress || "",
        paymentType: (inst as any).paymentInfo?.paymentType || "",
        rate: (inst as any).paymentInfo?.rate || "",
        overtimeRate: (inst as any).paymentInfo?.overtimeRate || "",
        deductions: (inst as any).paymentInfo?.deductions || "",
        taxId: (inst as any).paymentInfo?.taxId || "",
        paymentMethod: (inst as any).paymentInfo?.paymentMethod || "",
        payrollEmail: (inst as any).paymentInfo?.payrollEmail || "",
        payrollPhone: (inst as any).paymentInfo?.payrollPhone || "",
        idProof: (inst as any).paymentInfo?.idProof || null,
        rateType: (inst as any).paymentInfo?.rateType || "hourly",
        upiProvider: (inst as any).paymentInfo?.upiProvider || "",
        upiId: (inst as any).paymentInfo?.upiId || "",
      },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between p-6 pb-3">
          <h2 className="text-xl font-semibold text-[#5E35B1]">List of Instructors</h2>
        </div>
