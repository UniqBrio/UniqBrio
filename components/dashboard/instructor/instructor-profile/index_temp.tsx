...
export default function InstructorProfile({
    ...
    const handler = (e: Event) => {
      try {
        ...
        const mapped: Instructor[] = arr.map((it: any) => {
          ...
          // Find ALL matching courses by instructor name
          const matchingCourses = courses.filter(course => 
            course.instructor && course.instructor.toLowerCase() === (inst?.name || '').toLowerCase()
          )
          // Find ALL matching cohorts by instructor name
          const matchingCohorts = cohorts.filter(cohort => 
            cohort.instructor && cohort.instructor.toLowerCase() === (inst?.name || '').toLowerCase()
          )
          ...
          return {
            ...
            courseAssigned: courseNames || undefined,
            cohortName: cohortNames || undefined,
            courseIds: (inst as any)?.courseIds,
            cohortIds: (inst as any)?.cohortIds,
            cohortNames: uniqueCohortNames,
          } as Instructor
  ...
  const backfillFormFromInstructor = (inst: StoreInstructor): InstructorFormData => {
    ...
    return {
      ...
      certifications: (inst.qualifications as string[] | undefined) ?? [""],
      specializations: (inst.specializations as string[] | undefined) ?? [""],
      awards: [""],
      careerGoals: "",
      paymentInfo: {
        classCount: "",
