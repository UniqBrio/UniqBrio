...
export default function LeaveRequestForm({ onClose, draft }: LeaveRequestFormProps) {
  ...
  // Show post-create prompt to optionally open scheduling tool
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Show post-create prompt; close the form after user chooses
    setPostCreatePromptOpen(true)
  }
  ...
  <DialogContent className="max-w-2xl">
    ...
    {/* Post-create prompt to open scheduling service */}
    <AlertDialog open={postCreatePromptOpen} onOpenChange={setPostCreatePromptOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Reassign or reschedule dependencies?</AlertDialogTitle>
          <AlertDialogDescription>
            Do you want to reassign or reschedule for this instructor's dependency now?
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel onClick={() => { setPostCreatePromptOpen(false); onClose(); try { if (shouldReopenDrafts && typeof window !== 'undefined') window.dispatchEvent(new Event('instructor-leave-drafts:open')) } catch {} }}>No</AlertDialogCancel>
          <AlertDialogAction
            className="bg-purple-600 hover:bg-purple-700"
            onClick={() => {
              try {
                window.open("https://uniq-brio-service.vercel.app/services/schedule", "_blank", "noopener,noreferrer")
              } catch {}
              setPostCreatePromptOpen(false)
              onClose()
              try { if (shouldReopenDrafts && typeof window !== 'undefined') window.dispatchEvent(new Event('instructor-leave-drafts:open')) } catch {}
            }}
          >
            Yes, open scheduling
          </AlertDialogAction>
