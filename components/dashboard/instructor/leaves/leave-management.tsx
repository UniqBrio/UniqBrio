...
export default function LeaveManagement() {
  ...
  // Reopen drafts dialog automatically after a draft is converted to a request, if more drafts remain
  useEffect(() => {
    const handler = () => setDraftsOpen(true)
    if (typeof window !== 'undefined') {
      window.addEventListener('instructor-leave-drafts:open', handler)
    }
    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener('instructor-leave-drafts:open', handler)
      }
    }
  }, [])
  ...
  // Default: include all except optional course/cohort columns so they appear under 'Available'
  const DEFAULT_COLUMNS: LeaveColId[] = LEAVE_TABLE_COLUMNS
    .map(c => c.id)
    .filter(id => id !== ("courseName" as LeaveColId) && id !== ("cohortName" as LeaveColId))
  ...
  const formatInstructorId = (raw: string) => {
    ...
    if (instr) return `INSTR${parseInt(instr[1], 10).toString().padStart(4, '0')}`
