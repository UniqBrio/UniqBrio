...
import type { InvoiceFilters } from "./invoice-types"
import MultiSelectDropdown from "./MultiSelectDropdown"

interface InvoiceFiltersProps {
  selectedFilters: InvoiceFilters;
  setSelectedFilters: (filters: InvoiceFilters) => void;
...
export function InvoiceFiltersComponent({
  selectedFilters,
  setSelectedFilters,
  planTypeOptions = ["Monthly", "Yearly"],
  statusOptions = ["Paid", "Failed", "Pending", "Overdue"],
  paymentMethodOptions = ["Card", "UPI", "Net Banking", "Bank Transfer"],
}: InvoiceFiltersProps) {
  const [filterDropdownOpen, setFilterDropdownOpen] = useState(false);
  const [filterAction, setFilterAction] = useState<string | null>(null);
  const [pendingFilters, setPendingFilters] = useState<InvoiceFilters>(selectedFilters);
  ...
  const hasActiveFilters = () => {
    return (
      selectedFilters.planType.length > 0 ||
  ...
  const getActiveFilterCount = () => {
    let count = 0;
    if (selectedFilters.planType.length > 0) count++;
  ...
  const toggleSelection = (
    list: string[],
    value: string,
    key: keyof Pick<InvoiceFilters, 'planType' | 'status' | 'paymentMethod'>
  ) => {
    ...
    >
      ...
      >
        <div className="max-h-[32rem] overflow-y-auto p-4">
          {/* First Row: Plan Type and Status */}
          <div className="grid grid-cols-2 gap-3 mb-3">
            <MultiSelectDropdown
              label="Plan Type"
              options={planTypeOptions}
              selected={pendingFilters.planType}
              onChange={(next) => setPendingFilters({ ...pendingFilters, planType: next })}
            />
            <MultiSelectDropdown
              label="Status"
              options={statusOptions}
              selected={pendingFilters.status}
              onChange={(next) => setPendingFilters({ ...pendingFilters, status: next })}
            />
          </div>
