...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({
  ...
  return (
    <div className={`mb-4 relative ${className}`} ref={containerRef}>
      ...
      <button
        type="button"
        className="w-full text-left text-xs px-3 py-2 rounded-md border bg-white flex items-center justify-between gap-2 hover:bg-gray-50 transition-colors"
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div
          className={`absolute mt-1 w-full z-50 bg-white border rounded shadow-lg p-2 text-xs ${hasManyOptions ? 'max-h-[20rem] overflow-hidden' : 'max-h-56 overflow-y-auto'}`}
          role="listbox"
          aria-label={label}
        >
          ...
          {hasManyOptions && (
            <div className="px-2 pb-2">
              <input
                ...
                className="w-full text-xs px-2 py-1 border rounded focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>
          )}
              ...
              <div className={hasManyOptions ? "max-h-32 overflow-y-auto pr-1" : ""}>
                {filteredOptions.map(opt => {
                  ...
                  return (
                    <label
                      key={opt}
                      className="flex items-center gap-2 px-2 py-1 rounded cursor-pointer hover:bg-purple-50"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
