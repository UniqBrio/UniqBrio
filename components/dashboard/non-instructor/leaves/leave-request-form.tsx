...
export default function LeaveRequestForm({ onClose, draft }: LeaveRequestFormProps) {
    ...
    try {
      if (draft?.id) {
        ...
        if (result.ok) {
          ...
          setFormError(result.error || 'Failed to update draft. Please try again.')
          return
        }
      } else {
        // Create new draft
        ...
        if (result.ok) {
          ...
          setFormError(result.error || 'Failed to create draft. Please try again.')
          return
        }
      }
    } catch (err) {
      console.error('Error saving draft:', err)
      setFormError(err instanceof Error ? err.message : 'Failed to save draft. Please try again.')
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.instructorId) {
      errors.instructorId = "Select a non-instructor."
    }
    ...
    if (endDate < startDate) {
      setFieldErrors({ endDate: RANGE_ERROR })
    ...
    if (draft?.id) {
      ...
      try {
        ...
        if (!updateResult.ok) {
          ...
          setFormError(updateResult.error || 'Failed to update draft. Please try again.')
          return
        }
      } catch (err) {
