...
export function useInstructorFiltering({
  ...
  const filteredAndSortedInstructors = useMemo(() => {
    ...
    // Apply experience range filter whenever it differs from the default [0, 50].
    // This also supports negative minimum values correctly.
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case "id": {
          // Prefer numeric sort by extracting trailing digits if present, else fallback to string
          const num = (s: string) => {
            const m = s.match(/(\d+)/g)
            if (m && m.length) {
              // Use the last numeric group to avoid splitting on spaces
          ...
          const aNum = num(a.instructorId || '')
          const bNum = num(b.instructorId || '')
          if (!Number.isNaN(aNum) && !Number.isNaN(bNum)) {
            aVal = aNum
            bVal = bNum
          } else {
            aVal = (a.instructorId || '').toLowerCase()
            bVal = (b.instructorId || '').toLowerCase()
          }
          break
        }
        case "name":
        ...
        case "gender":
          ...
          break
        case "contractType":
          aVal = (a.contractType || '').toLowerCase()
          bVal = (b.contractType || '').toLowerCase()
          break
        case "jobLevel":
          aVal = (a.jobLevel || '').toLowerCase()
          bVal = (b.jobLevel || '').toLowerCase()
          break
        case "courseAssigned":
...
export function useInstructorActions({ instructors, setInstructors }: UseInstructorActionsProps) {
  ...
  toast({ title: 'Nothing to export', description: 'No non-instructors to export.' })
      return
    }
    ...
    if (!rows.length) {
  toast({ title: 'Nothing selected', description: 'No selected non-instructors to export.' })
      return
    }

  const csvHeader = "ID,Non Instructor Name,Role,Gender,Experience (Years),Email,Phone,Contract Type,Job Level,Date of Birth,Joining Date\n"
    ...
    // Filename doubles as Excel sheet name when opening CSV
    a.download = `non-instructors_export_${date}.csv`
    a.click()
      ...
      reader.onload = (e) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const instructor: Instructor = {
            instructorId: values[findIdx(['ID'])] || `NON INS${String(Date.now()).slice(-3)}`,
            name: values[findIdx(['Non Instructor Name','Name'])] || '',
            role: values[findIdx(['Role'])] || '',
            gender: values[findIdx(['Gender'])] || '',
            yearsOfExperience: (() => {
              const idx = findIdx(['Experience (Years)', 'Experience'])
              const parsed = idx >= 0 ? parseInt(values[idx]) : NaN
              return Number.isNaN(parsed) ? 0 : parsed
            })(),
            email: values[findIdx(['Email'])] || '',
            phone: values[findIdx(['Phone'])] || '',
        ...
        setInstructors(prev => [...prev, ...newInstructors])
          toast({ title: 'Import complete', description: `Imported ${newInstructors.length} non-instructors.` })
      }
