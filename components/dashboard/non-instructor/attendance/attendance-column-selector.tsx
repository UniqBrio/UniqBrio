...
export type AttendanceColLabel =
  | "Non-Instructor ID"
  | "Non-Instructor Name"
  | "Date"
  | "Status"
...
export const ATTENDANCE_TABLE_COLUMNS: { id: AttendanceColLabel; label: string }[] = [
  { id: "Non-Instructor ID", label: "Non-Instructor ID" },
  { id: "Non-Instructor Name", label: "Non-Instructor Name" },
  { id: "Date", label: "Date" },
...
const NON_EDITABLE_COLS: AttendanceColLabel[] = ["Non-Instructor ID", "Non-Instructor Name"]
...
export default function AttendanceColumnSelector({ value, onChange, storageKey = "attendanceDisplayedColumns", className, buttonTitle = "Select displayed columns" }: Props) {
  ...
  const [draft, setDraft] = useState<AttendanceColLabel[]>(() => {
    const base = (value || [])
      .filter(v => ATTENDANCE_TABLE_COLUMNS.some(c => c.label === v)) as AttendanceColLabel[]
    NON_EDITABLE_COLS.forEach(c => { if (!base.includes(c)) base.push(c) })
    return base
  })
  const [selectedAvailable, setSelectedAvailable] = useState<AttendanceColLabel[]>([])
  const [selectedDisplayed, setSelectedDisplayed] = useState<AttendanceColLabel[]>([])
  ...
  const available = useMemo(
    () => ATTENDANCE_TABLE_COLUMNS.map(c => c.id).filter(id => !draft.includes(id) && !NON_EDITABLE_COLS.includes(id)) as AttendanceColLabel[],
    [draft]
  )

  // Keep user's chosen order; only validate and de-duplicate
  const sanitizeOrder = (cols: AttendanceColLabel[]): AttendanceColLabel[] => {
    const seen = new Set<AttendanceColLabel>()
    const valid = cols.filter(
      (c): c is AttendanceColLabel => ATTENDANCE_TABLE_COLUMNS.some(x => x.id === c) && (seen.has(c) ? false : (seen.add(c), true))
    )
    // Ensure mandatory columns remain present; if missing, insert at their default relative positions
    NON_EDITABLE_COLS.forEach(c => {
      if (!valid.includes(c)) {
        const defIdx = ATTENDANCE_TABLE_COLUMNS.findIndex(x => x.id === c)
        let insertAt = valid.length
        for (let i = 0; i < valid.length; i++) {
          const curIdx = ATTENDANCE_TABLE_COLUMNS.findIndex(x => x.id === valid[i])
  ...
  const handleAdd = () => {
    ...
    window.dispatchEvent(new CustomEvent('attendance-displayed-columns-changed', { detail: next }))
  }
  ...
  const reset = () => {
    const allColumns = ATTENDANCE_TABLE_COLUMNS.map(c => c.id as AttendanceColLabel)
    setDraft(allColumns)
    setSelectedAvailable([])
    setSelectedDisplayed([])
  }
                ...
                >
                  {available.map((col, idx) => (
                    ...
                    >
                      ...
                      />
                      <span>{ATTENDANCE_TABLE_COLUMNS.find(c => c.id === col)?.label ?? col}</span>
                    </label>
                  ))}
                </div>
              </div>
                  ...
                  {draft.map((col, idx) => {
                    ...
                    return (
                      ...
                      >                        
                        ...
                        <span>
                          {ATTENDANCE_TABLE_COLUMNS.find(c => c.id === col)?.label ?? col}
                          {mandatory && ' *'}
                        </span>
                      </label>
                    )
