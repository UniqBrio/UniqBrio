...
export default function AttendanceSearchFilters({
  ...
  React.useEffect(() => {
    ...
    if (typeof window === 'undefined') return;
    try {
      const NEW_KEY = 'nonInstructorAttendanceDisplayedColumns';
      ...
      if (Array.isArray(parsed) && parsed.length) {
        ...
        let migrated = (parsed as string[]).map((c: string) =>
          c === 'Notes' ? 'Remarks'
          : c === 'Instructor ID' ? 'Non-Instructor ID'
          : c === 'Instructor Name' ? 'Non-Instructor Name'
          : c
        );
        const must = ['Non-Instructor ID', 'Non-Instructor Name'];
    ...
    <div>
      <div className="flex flex-col lg:flex-row gap-2 mb-4">
        ...
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search non-instructors or remarks..."
            className="pl-10"
            value={searchTerm}
      ...
      <div className="flex items-center justify-between mb-4 bg-purple-50 rounded-lg px-4 py-2">
        ...
        {typeof displayedColumns !== 'undefined' && typeof setDisplayedColumns !== 'undefined' && (
          <AttendanceColumnSelector
            ...
            storageKey="nonInstructorAttendanceDisplayedColumns"
