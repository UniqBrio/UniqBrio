...
import { useNonInstructorDrafts, NonInstructorDraft } from "@/hooks/dashboard/staff/use-non-instructor-drafts"
...
export default function InstructorDraftsDialog({ 
  ...
  const { drafts, deleteDraft, updateDraftName, draftsCount, loadDrafts } = useNonInstructorDrafts()
  ...
  const handleEditName = (draft: NonInstructorDraft) => {
  ...
  const handleLoadDraft = (draft: NonInstructorDraft) => {
  ...
  // Note: We previously displayed a badge for draft.level and a raw draft.id line.
  // These were default placeholders not sourced from any form input, so they have
  // been removed from the UI to avoid confusion.
