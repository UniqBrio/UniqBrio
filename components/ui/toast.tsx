// d:\UB\components\ui\toast.tsx
"use client"

import * as React from "react"
import * as ToastPrimitives from "@radix-ui/react-toast"
import { cva, type VariantProps } from "class-variance-authority"
import { X, CheckCircle, AlertTriangle } from "lucide-react" // Import icons
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    // Viewport positioning is handled in toaster.tsx now
    className={cn(
...
const toastVariants = cva(
  // Base styles - adjusted padding slightly
  "group pointer-events-auto relative flex w-full items-center space-x-4 overflow-hidden rounded-md border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        // Add Success Variant using CSS variables
        success:
...
// Update VariantProps type to include 'success'
type ToastVariantProps = VariantProps<typeof toastVariants>;

const Toast = React.forwardRef<
  React.ElementRef<typeof ToastPrimitives.Root>,
  React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root> &
    ToastVariantProps // Use updated type
>(({ className, variant, children, ...props }, ref) => {
  ...
  const Icon = variant === 'success'
    ...
    : variant === 'destructive'
    ? AlertTriangle
    : null; // No icon for default variant, or choose one
  ...
  return (
    ...
    >
      ...
      {/* Wrap content to allow icon spacing */}
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ...
    className={cn(
      ...
      // Success Action Styles (Optional - adjust as needed)
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ...
    className={cn(
      ...
      // Destructive Close Styles
      "group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600",
      // Success Close Styles
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    // Added margin-bottom for spacing if description exists
    className={cn("text-sm font-semibold [*:has(~_[data-description])_&]:mb-1", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    data-description // Add data attribute for styling hook in title
...
export {
  ...
  ToastProvider,
  ToastViewport, // Exporting viewport from here is fine, but positioning is controlled in toaster.tsx
  Toast,
  ToastTitle,
  ToastDescription,
  ToastClose,
