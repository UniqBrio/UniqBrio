...
import { NonInstructorLeaveDraft } from "@/lib/dashboard/staff/models"
...
// GET - Retrieve all draft leave requests for non-instructors
  ...
  return runWithTenantContext(
    ...
    async () => {
      try {
        ...
        const drafts = await NonInstructorLeaveDraft.find({ tenantId: session.tenantId }).sort({ createdAt: -1 }).lean()
        return NextResponse.json({ ok: true, data: drafts })
      } catch (err: any) {
        console.error("/api/dashboard/staff/non-instructor/leave-drafts GET error", err)
  ...
  return runWithTenantContext(
    ...
    async () => {
      try {
        ...
        const created = await NonInstructorLeaveDraft.create(body)
        return NextResponse.json({ ok: true, data: created })
      } catch (err: any) {
        console.error("/api/non-instructor-leave-drafts POST error", err)
  ...
  return runWithTenantContext(
    ...
    async () => {
      try {
        ...
        const updated = await NonInstructorLeaveDraft.findOneAndUpdate(
        ...
        console.error("/api/non-instructor-leave-drafts PUT error", err)
  ...
  return runWithTenantContext(
    ...
    async () => {
      try {
        ...
        const result = await NonInstructorLeaveDraft.deleteOne({ id, tenantId: session.tenantId })
        if (result.deletedCount === 0) {
          return NextResponse.json({ ok: false, error: 'Draft not found' }, { status: 404 })
        }
        return NextResponse.json({ ok: true })
      } catch (err: any) {
        console.error("/api/non-instructor-leave-drafts DELETE error", err)
