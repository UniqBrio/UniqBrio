...
export async function POST(req: Request) {
  try {
    ...
    // Find the draft
    const draft = await InstructorLeaveDraft.findOne({ id: draftId }).lean()
    if (!draft) {
    ...
    try {
      ...
      const inst = await Instructor.findOne({ id: draft.instructorId }).lean()
  ...
  // Create the leave request
    ...
    await InstructorLeaveDraft.deleteOne({ id: draftId })
